<template>
  <div class="top">
    <el-button
      ...
      @click="revertBatch"
    >
      <span class="iconfont icon-revert"></span><span>还原</span>
    </el-button>
    <el-button
      type="danger"
      :disabled="selectFileIdList.length == 0"
      @click="delBatch"
    >
      <span class="iconfont icon-del"></span><span>批量删除</span>
    </el-button>
  </div>
...
// 还原http请求
const revertDone = async (row) => {
  const confirmMsg = row
    ? `你确定要还原【${row.fileName}】吗？`
    : `你确定要还原这些文件吗？`;
  proxy.Confirm(confirmMsg, async () => {
    let result = await proxy.Request({
      url: api.recoverFile,
      params: {
...
// 删除http请求
const delDone = async (row) => {
  const confirmMsg = row
    ? `你确定要删除【${row.fileName}】吗？`
    : `你确定要删除选中的文件吗？删除将无法恢复`;
  proxy.Confirm(confirmMsg, async () => {
    let result = await proxy.Request({
      url: api.delFile,
      params: {
